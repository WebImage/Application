...
class ViewFactory implements ContainerAwareInterface {
...
//		$this->callCreator($view = new View($this, $this->getEngineFromPath($path), $view, $path, $data));
		...
		if ($foundView === null) {
			$debugViews = is_array($view) ? implode(', ', $view) : $view;
			throw new ViewNotFoundException(sprintf('Unable to find view: %s', $debugViews));
		}
		...
		$this->triggerEvent('view.created', $view);
	...
	private function triggerEvent(string $event, $view): array
	{
		...
		return $em->trigger($event, $view, $this);
	}
	...
	public function findView($view): ?FoundView
	{
		...
		foreach($views as $checkView) {
			$responses = $this->triggerEvent('view.find', $view);
